    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-neutral-500 text-center mb-8">
          ...
          {isSignIn && (
            <button onClick={() => switchMode('signup')} className="text-primary-600 underline font-medium hover:text-primary-800">
              Sign up
            </button>
          )}
          {isSignUp && "Already have an account? "}
          {isSignUp && (
            <button onClick={() => switchMode('signin')} className="text-primary-600 underline font-medium hover:text-primary-800">
              Sign in
            </button>
          )}
          {isReset && (
        ...
        {!isReset && (
          <>
            <Button variant="secondary" loading={loading} onClick={handleGoogle} className="w-full mb-4" ariaLabel="Continue with Google">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          {isSignUp && (
            <div>
              <label htmlFor="confirm" className="block text-sm font-medium text-neutral-700 mb-1">Confirm Password</label>
              <input
          ...
          {error && (
            <p role="alert" aria-live="assertive" className="text-sm text-error-600 bg-error-50 border border-error-200 rounded-lg px-3 py-2">
              {error}
            </p>
          )}
          {info && (
            <p role="status" aria-live="polite" className="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
              {info}
            </p>
          )}
        ...
        <p className="text-xs text-neutral-500 text-center mt-6">
          ...
          <Link to="/privacy" className="text-neutral-700 underline hover:text-neutral-900 focus:outline-2 focus:outline-primary-600 rounded">
            Privacy Policy
          </Link>.
        </p>
      </div>
