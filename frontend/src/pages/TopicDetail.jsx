...
export default function TopicDetail() {
  ...
  useEffect(() => {
    ...
    topicsApi.getBySlug(slug)
      .then((data) => { setTopic(data); document.title = `${data.title} | Electra`; })
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      <Link to="/topics" className="text-sm text-primary-600 hover:text-primary-800 hover:underline focus:outline-2 focus:outline-primary-600 rounded mb-6 inline-block">
        ← Back to Topics
      </Link>
      ...
      <article>
        <header className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {topic.country.map((c) => <span key={c} className="bg-neutral-100 text-neutral-600 text-xs px-2 py-0.5 rounded-full">{c}</span>)}
          </div>
        </header>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <ul className="list-disc list-inside space-y-1">
              {topic.prerequisites.map((slug) => (
                <li key={slug}><Link to={`/topics/${slug}`} className="text-primary-600 hover:underline text-sm focus:outline-2 focus:outline-primary-600 rounded capitalize">{slug.replace(/-/g, ' ')}</Link></li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div className="border-t border-neutral-200 pt-8">
          {user ? (
            ...
            <div>
              <p className="text-sm text-neutral-500 mb-3">Sign in to take the quiz and track your progress.</p>
              <Link to="/login" state={{ from: { pathname: `/quiz/${slug}` } }} className="inline-flex items-center gap-2 px-6 py-3 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 focus:outline-2 focus:outline-primary-600 transition-colors">
