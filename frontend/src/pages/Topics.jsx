...
const CATEGORIES = ['all', 'registration', 'eligibility', 'ballot', 'campaign', 'counting', 'dispute'];
...
export default function Topics() {
  ...
  useEffect(() => { document.title = 'Topics | Electra'; }, []);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {/* Category tabs */}
      <div role="tablist" aria-label="Filter topics by category" className="flex flex-wrap gap-2 mb-6">
        {CATEGORIES.map((cat) => (
          <button
            key={cat}
            role="tab"
          ...
          {filtered.map((topic) => (
            ...
            >
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {topic.country.map((c) => <Badge key={c} variant="neutral">{c}</Badge>)}
              </div>
