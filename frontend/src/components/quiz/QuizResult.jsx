...
export default function QuizResult({ score, total, correct, results, onRetry, onNextTopic }) {
  useEffect(() => {
    announceToScreenReader(
      `Quiz complete. You scored ${score}% - ${correct} out of ${total} correct.`
    )
  }, [score, total, correct])

  const scoreColor =
    score >= 80 ? 'text-success-600' : score >= 50 ? 'text-yellow-600' : 'text-error-600'

  const scoreLabel = score >= 80 ? 'Excellent!' : score >= 50 ? 'Good effort' : 'Keep practising'
  ...
  return (
    <div className="bg-white rounded-xl border border-neutral-200 p-8 shadow-sm max-w-2xl mx-auto">
        ...
        {results.map((r, i) => (
          ...
          >
            <div className="flex items-start gap-3">
              <span
                className={`text-lg mt-0.5 ${r.correct ? 'text-success-600' : 'text-error-600'}`}
                aria-hidden="true"
              >
                {r.correct ? '✓' : '✗'}
              </span>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button variant="primary" onClick={onNextTopic} className="flex-1">
          Continue Learning
        </Button>
      </div>
    </div>
...
QuizResult.propTypes = {
  ...
  onNextTopic: PropTypes.func.isRequired,
}
