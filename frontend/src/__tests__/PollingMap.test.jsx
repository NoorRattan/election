  ...
  const MockGeocoder = vi.fn(function MockGeocoder() {
    return {
      geocode: vi.fn((_, callback) =>
        callback([{ geometry: { location: { lat: () => 51.5, lng: () => -0.1 } } }], 'OK')
      ),
    }
  })
...
describe('PollingMap - primary CTA (official link)', () => {
  ...
  it('official polling station link is visible for UK country', () => {
    ...
    const link =
      screen.queryByRole('link', { name: /official|find polling|find your/i }) ||
  ...
  it('official link has meaningful text (not just a raw URL)', () => {
    ...
    const officialLink = links.find((l) => l.href && l.href.includes('gov.uk'))
    if (officialLink) {
...
describe('PollingMap - search form', () => {
  ...
  it('address search input is present with an associated label', () => {
    ...
    const input =
      screen.queryByRole('textbox') || screen.queryByLabelText(/address|postcode|search/i)
...
describe('PollingMap - API load failure (graceful degradation)', () => {
  ...
  it('official link is still visible even when Maps API fails to load', () => {
    ...
    const hasExternalLink = Array.from(links).some(
      (l) =>
        l.href &&
        (l.href.includes('gov.uk') || l.href.includes('usa.gov') || l.href.includes('eci.gov'))
    )
