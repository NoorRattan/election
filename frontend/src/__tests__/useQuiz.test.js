...
describe('useQuiz', () => {
  ...
  it('status becomes active after loadQuiz resolves', async () => {
    ...
    quizApi.getQuestions.mockResolvedValueOnce({
      questions: [{ id: 'q1', question: 'Q?', options: ['A', 'B', 'C', 'D'], country: ['ALL'] }],
    })
    const { result } = renderHook(() => useQuiz())
    await act(async () => {
      await result.current.loadQuiz('voter-registration')
    })
  ...
  it('selectAnswer updates selectedAnswers map', async () => {
    ...
    const { result } = renderHook(() => useQuiz())
    await act(async () => {
      await result.current.loadQuiz('test')
    })
    act(() => {
  ...
  it('status becomes results after submitQuiz', async () => {
    ...
    await act(async () => {
      await result.current.submitQuiz('test')
    })
