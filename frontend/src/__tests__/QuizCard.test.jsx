...
describe('QuizCard', () => {
  it('renders the question text', () => {
    render(<QuizCard question={question} selectedIndex={null} onSelect={() => {}} questionNumber={1} totalQuestions={5} />);
  ...
  it('calls onSelect with correct index when option clicked', () => {
    ...
    render(<QuizCard question={question} selectedIndex={null} onSelect={onSelect} questionNumber={1} totalQuestions={5} />);
    fireEvent.click(screen.getByDisplayValue('2'));
    expect(onSelect).toHaveBeenCalledWith(2);
  });

  it('marks the selected option as checked', () => {
    render(<QuizCard question={question} selectedIndex={2} onSelect={() => {}} questionNumber={1} totalQuestions={5} />);
    expect(screen.getByDisplayValue('2')).toBeChecked();
  });

  it('renders a fieldset with a legend', () => {
    const { container } = render(<QuizCard question={question} selectedIndex={null} onSelect={() => {}} questionNumber={1} totalQuestions={5} />);
