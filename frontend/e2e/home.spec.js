...
test.describe('Home page', () => {
  ...
  test('h1 "Learn How Elections Work" is visible', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: /learn how elections work/i, level: 1 })
    ).toBeVisible()
  })
  ...
  test('Navbar has "Timeline" link visible', async ({ page }) => {
    await expect(
      page.getByRole('navigation').getByRole('link', { name: /timeline/i })
    ).toBeVisible()
  })

  test('Navbar does NOT have a "Quiz" link', async ({ page }) => {
  ...
  test('country selector cards are present (UK, US, India) when no country is set', async ({
    page,
  }) => {
  ...
  test('Footer has "Accessibility Statement" link that navigates to /accessibility', async ({
    page,
  }) => {
    const a11yLink = page
